...
namespace TsuruLauncher.Services
{
    public class GameService
    {
        /// <summary>
        /// 每个游戏版本的 JVM / 内存配置文件名。
        /// v2.0 起由 <c>lyzl_profile.json</c> 改名为 <c>tsuru_profile.json</c>
        /// （项目从 LYZL 改名为 Tsuru Launcher）。
        /// </summary>
        public const string ProfileFileName = "tsuru_profile.json";

        /// <summary>v1.x 时代的旧文件名，仅用于兼容读取。</summary>
        public const string LegacyProfileFileName = "lyzl_profile.json";

        /// <summary>
        /// 解析某个游戏版本的配置文件路径：优先用新名字；只有当新文件不存在、
        /// 而旧文件还在时，才返回旧路径 —— 保证老用户（目录里还留着
        /// lyzl_profile.json）的 JVM / 内存设置不会因为改名而丢失。
        /// 下次保存时会写到新文件名并删掉旧文件（见 <see cref="SaveVersionConfig"/>）。
        /// </summary>
        public static string ResolveProfilePath(string gameDir)
        {
            string newPath = Path.Combine(gameDir, ProfileFileName);
            if (File.Exists(newPath)) return newPath;
            string legacyPath = Path.Combine(gameDir, LegacyProfileFileName);
            if (File.Exists(legacyPath)) return legacyPath;
            return newPath;   // 两个都没有 → 用新名字（保存时创建）
        }
        ...
        public GameInstance? ParseVersionJson(string jsonPath, string dotMinecraftPath, bool isIsolationEnabled)
        {
            try
            {
                ...
                try 
                {
                    ...
                    // Strip loader suffixes first ("1.20.5-fabric-0.15.11" -> "1.20.5")
                    // so the minor/patch heuristics see the bare MC version.
                    string mcCore = Regex.Match(id, @"^\d+\.\d+(\.\d+)?").Value;
                    var versionParts = string.IsNullOrEmpty(mcCore) ? id.Split('.') : mcCore.Split('.');
        ...
        public void LoadVersionConfig(GameInstance instance)
        {
            try
            {
                string configPath = ResolveProfilePath(instance.GameDir);
        ...
        public void SaveVersionConfig(GameInstance instance)
        {
            try
            {
                ...
                string configPath = Path.Combine(instance.GameDir, ProfileFileName);
                File.WriteAllText(configPath, json.ToString());

                // 迁移：旧名字的文件已经不需要了，删掉，避免下次又读到旧数据
                try
                {
                    string legacyPath = Path.Combine(instance.GameDir, LegacyProfileFileName);
